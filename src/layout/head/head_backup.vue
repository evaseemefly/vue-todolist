            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav">
                    <li v-for="(group,index) in groups" :class="index==group_index?'active':''">
                        <a href="#" @click="changeIndex(index)">
                            {{group.department.derpartmentname}}
                        </a>
                        <!-- <router-link to="/home/content/"+group.did>
                                <span class="sr-only">(current)</span>
                            </router-link> -->
                        <!-- <router-link to="{path:'/home/content/',query{did:group.did}'}">
                                <span class="sr-only">(current)</span>
                            </router-link> -->
                        <!-- <router-link to="/home/content/4">
                                <span class="sr-only">(current)</span>
                            </router-link> -->
                        <!-- <router-link to="/home/content/4" @click="changeIndex(index)>
                        {{group.department.derpartmentname}}
                            <span class=" sr-only ">(current)</span>
                        </router-link> -->
                        <!-- <router-link to="/home/content/4">
                            <span class="glyphicon glyphicon-user"></span>&nbsp; {{group.department.derpartmentname}}
                        </router-link> -->
                    </li>
                    <li class="dropdown ">
                        <a href="# " class="dropdown-toggle " data-toggle="dropdown " role="button " aria-haspopup="true
                            " aria-expanded="false ">当前部门
