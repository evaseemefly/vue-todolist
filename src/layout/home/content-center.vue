...
export default {
    ...
    init_control() {
      ...
      $(".my_select_user").editable({
        ...
        url: function(params) {
          ...
          /*
            9-20 修改内容：
            每次新增一行数据后，table_data中的数据未更新，导致再次初始化table时，仍会显示之前的默认值
            思路：
              -1 获取当前行的日期
              -2 获取当前列的field（duty的名字）
              -3 根据当前行的日期从table_data中找到该日期的obj，并获取索引
              -4 根据该索引获取到该obj，
                 并根据field修改对应的值
            */
          ...
          // 根据日期找到修改的值
          var selectDutyDate = myself.curRow.dutydate;

          // 获取修改的指定的列
          var targetCellColumn = myself.curCell.field;

          // 获取当前行在table_data中的索引
          var targetDataIndex = myself.table_data.findIndex(
            (val, index, num) => {
              return val.dutydate === selectDutyDate;
            }
          );
          myself.table_data[targetDataIndex][targetCellColumn] = params.value;
        },
  ...
  beforeDestroy() {
    bus.$off("on-loadTable");
    bus.$off("on-clearData");
  }
};
</script>
